...
void OnGlfwKeyboardEvent(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    switch (key) {
        ...
        case GLFW_KEY_9: key = K_9; break; case GLFW_KEY_LEFT_CONTROL: key = K_CNTRL; break;
        ...
        case GLFW_KEY_Y: key = K_Y; break; case GLFW_KEY_Z: key = K_Z; break; case GLFW_KEY_LEFT_SHIFT: key = K_SHIFT; break;
...
void OnGlfwResizeFramebuffer(GLFWwindow* window, int width, int height)
{
    ...
    //Final render target
    glDeleteTextures(1, &app->finalAttachmentHandle);
